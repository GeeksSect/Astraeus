		...
		int16_t * _pitch_curr, int16_t * _roll_curr, uint16_t d_t)
{
	...
	roll_prev = roll_curr;
	pitch_curr = (49*((*gx * d_t/10000) + pitch_prev)/50) +  (*acell_pitch / 50);
	roll_curr = (49*((*gy*(-1) * d_t /10000) + roll_prev)/50) +  (*acell_roll/50);
...
void my_ESC(int16_t * pitch, int16_t * roll, int16_t * pow, int16_t * force, int16_t * gx, int16_t * gy, uint16_t d_t)
{
  ...
  }
  {
	I_p += *pitch * d_t;
	I_r += *roll * d_t;
	pow[0] = *force + (*pitch + *roll) *Kp + (*gx +(-*gy))*Kd + (I_p+I_r)*Ki/10000;
    pow[1] = *force + (-*pitch + *roll)*Kp + (-*gx +(-*gy))*Kd + (-I_p+I_r)*Ki/10000;
    pow[2] = *force + (-*pitch - *roll)*Kp + (-*gx -(-*gy))*Kd + (-I_p-I_r)*Ki/10000;
    pow[3] = *force + (*pitch - *roll)*Kp  + (*gx -(-*gy))*Kd + (I_p-I_r)*Ki/10000;
